...
class ProviderContainer:
    ...
    def get_data(self, pd_obj):
        if pd_obj.group == "provider":
            ...
            self._handle_template_data(pd_obj)

        for child in pd_obj.children:
            if (
                child.group in ["job", "test plan", "template"]
...
def go_through_provider(provider_root):
    print("## Start to browse objects from provider")
    obj_pd_container = ProviderContainer()
    obj_pd_container.get_data(provider_root)
    print("## End to browse objects from provider")

    return obj_pd_container.provider_data


def dump_test_plans(provider):
    sa = SessionAssistant(
    ...
    if not provider:
        raise SystemExit("provider variable should not be empty")

    target_provider = [pd for pd in installed_providers if pd.name == provider]

    if not target_provider:
        raise SystemExit(
            f"{provider} are not available in current environment"
        )

    print("## Browsing...")
    root = ExplorerMore(target_provider).get_object_tree()
    return go_through_provider(root)
...
def generate_job_readme(root_dir, provider_data):

    sorted_cases, all_cases = sort_by_directory_for_environ(provider_data)
    os.chdir(root_dir)

    for key in sorted_cases.keys():
        _, tc_category = os.path.split(key)

        os.makedirs(tc_category)
        string = render_readme_with_env(
            tc_category, sorted_cases.get(key, {}), all_cases
        )

        output_file = os.path.join(f"{tc_category}", "cases_and_environ.md")
        print(f"Creating the {output_file}")
...
def main():
    ...
    parser.add_argument(
        "provider_dir",
        help="the root directory of specified provider",
    )
    parser.add_argument(
    ...
    provider_data = dump_test_plans(args.provider_name)

    os.makedirs(args.output_dir, exist_ok=True)
    os.chdir(args.output_dir)

    unit_folder = os.path.join(args.provider_dir, "unit")
    os.makedirs(unit_folder, exist_ok=True)
    generate_job_readme(unit_folder, provider_data[args.provider_name])
